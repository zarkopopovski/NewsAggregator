...
public class Kanal5Driver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.getElementsByClass("vesticat_naslov");
			...
			if (links != null && links.size() > 0) {
				...
				for (Element element : links) {
					...
					if (newsLink != null && !newsLink.equals("")) {
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements titleElement = doc.select("div[class=title]").select("h2");
			String newsTitle = titleElement.text();
			Elements newsElements = doc.select("div[class=content]");
			if (newsElements != null) {
				String[]dataAsArray = newsElements.toString().split("\n");
				...
				for (int i = 0; i < dataAsArray.length; i++) {
					...
					if (dataAsArray[i].contains("</script>")) {
						
						for (int j = (i + 1); j < dataAsArray.length; j++) {
							
							if (dataAsArray[j].contains("<script>")) {
								stringFound = true;
								break;
							}
							
							bodyBuffer.append(dataAsArray[j]);
				...
				try {
					...
					parsetData.setNewsTitle(newsTitle);
					parsetData.setNewsBody(bodyBuffer.toString());
