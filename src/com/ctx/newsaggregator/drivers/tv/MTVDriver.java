...
public class MTVDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("div[class=box]").select("a");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements titleElement = doc.select("h1[class=naslov]");
			Elements newsElements = doc.select("div[class=pane-content]");
