...
public class EkonomskiDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("h2[class=entry-title]").select("a");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements titleElement = doc.select("meta[property=og:title]");
			String titleName = titleElement.attr("content");
			
			Elements newsElements = doc.select("div[class=entry entry-content]");
