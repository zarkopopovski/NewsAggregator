...
public class SkyDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("a[class=readmore]");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements titleElement = doc.select("h2");
			String newsTitle = titleElement.text();
			
			Elements newsElements = doc.select("p");
			newsElements.select("object, img, a, span").remove();
