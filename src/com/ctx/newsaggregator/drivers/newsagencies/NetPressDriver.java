...
public class NetPressDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("div[class=more-infobox]").select("a[rel=bookmark]");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements subTitle = doc.select("h2[class=podnaslov]");
