...
public class DWDriver implements IDriver {
	
	private static final Integer ADDITIONAL_TIMEOUT_TIME = 2;
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Document doc = Jsoup.connect(categoryURL).timeout((Constants.MAX_DELAY_TIME + ADDITIONAL_TIMEOUT_TIME) * 1000).get();
			
			Elements links = doc.select("div[class=col2]").select("a");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements introElements = doc.select("p[class=intro]");
			
			String introText = "";
			if (introElements.size() > 0) {
				introText = introElements.text();
			}
			
			Elements newsElements = doc.select("div[class=longText]");
			...
			if (newsElements != null) {
				...
				try {
					...
					parsetData.setNewsBody(introText+" "+newsElements.text());
