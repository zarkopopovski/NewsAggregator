...
public class MKDMKDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("div[class=box]");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements subTitleElement = doc.select("div[class=field-teaser]");
			Elements newsElements = doc.select("div[class=field-body]");
