...
public class Puls24Driver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("div[class=category_news]");
			...
			if (links != null && links.size() > 0) {
				...
				for (Element element : links) {
					...
					Element newsUrl = aLinks.get(0);
					
					String newsLink = aLinks.toString();
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements wholeNewsElements = doc.select("div[class=article_post]");
			Elements titleElement = wholeNewsElements.select("h2");
