...
public class Kanal77Driver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("a[class=k2ReadMore]");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements titleElement = doc.select("h2[class=itemTitle]");
			Elements newsContainer = doc.select("div[class=itemFullText]");
			Elements newsElements = newsContainer.select("p");
