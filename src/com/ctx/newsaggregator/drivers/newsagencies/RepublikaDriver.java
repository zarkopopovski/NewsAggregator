...
public class RepublikaDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("a[rel=bookmark]");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements podnaslovElement = doc.select("h2[class=podnaslov]");
			Elements newsElements = doc.getElementsByTag("p");
			...
			if (newsElements != null) {
				...
				try {
					...
					parsetData.setNewsBody(podnaslovElement.text() + " " + newsElements.text());
