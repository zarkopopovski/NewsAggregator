...
public class DnevnikDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("a[class=more-link-kategorija]");
			...
			if (links != null && links.size() > 0) {
				...
				for (Element element : links) {
					...
					String newsLink = element.attr("href").trim();
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements newsElements = doc.getElementsByClass("WB_DNEVNIK_Normal");
