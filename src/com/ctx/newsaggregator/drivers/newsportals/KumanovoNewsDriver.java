...
public class KumanovoNewsDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("article").select("a");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			if (titleName.contains("È")) {
				titleName = titleName.substring((titleName.indexOf("È") + 1)).trim();
			}
			
			Elements newsElements = doc.select("div[class=page]").select("p");
			newsElements.select("a, img, script, xml, iframe").remove();
