...
public class OhridNewsDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("article[class=bs]").select("div[class=title]").select("a");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			if (titleName.contains(":")) {
				titleName = titleName.substring((titleName.indexOf("::") + 2)).trim();
			}
			
			Elements newsElements = doc.select("article").select("p");
