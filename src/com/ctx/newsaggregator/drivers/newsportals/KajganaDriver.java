...
public class KajganaDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("div[class=views-field-title]");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			if (newsTitle.contains("|")) {
				newsTitle = newsTitle.substring(0, newsTitle.indexOf("|"));
			}
			
			Elements newsElements = doc.select("div[class=node node-statija]").select("p");
