...
public class OknoDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("div[class=naslov]").select("a");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements newsElements = doc.select("div[id=node_container]").select("p");
			newsElements.select("a, img, script").remove();
