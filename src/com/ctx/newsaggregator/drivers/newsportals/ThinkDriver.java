...
public class ThinkDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements topLink = doc.select("div[id=colorcontentwrap4]").select("p").select("a");
			Elements links = doc.select("div[class=featbig]").select("p").select("a");

			if (topLink != null && topLink.size() > 0) {
				linksByCategoryList = new ArrayList<String>();	
				
				Element el = topLink.get(0);
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			String titleName = titleElement.get(0).text();
