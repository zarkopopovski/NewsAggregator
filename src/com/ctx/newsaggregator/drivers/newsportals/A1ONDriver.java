...
public class A1ONDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("a[class=article-permalink]");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements newsElements = doc.select("div[class=entry-content]").select("p");
			newsElements.select("a, img, script, xml, input, label, textarea, iframe, noscript").remove();
