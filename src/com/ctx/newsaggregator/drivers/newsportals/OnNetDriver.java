...
public class OnNetDriver implements IDriver {
	...
	public List<String> parseCategory(String categoryName, String categoryURL) {
		...
		try {
			...
			Elements links = doc.select("div[class=vovedSodrzina]");
	...
	public NewsEntity parseNewsPerCategory(String newsURL) {
		...
		try {
			...
			Elements divNews = doc.select("div[class=statija]");
			...
			Elements subTitle = divNews.select("div[class=vovedVoStatija]");
			Elements newsElements = divNews.select("p");
			newsElements.select("img, a, em").remove();
